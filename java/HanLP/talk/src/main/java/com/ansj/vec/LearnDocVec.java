...
public class LearnDocVec {
	...
	public void saveModel(File file) {
		...
		try (DataOutputStream dataOutputStream = new DataOutputStream(
			...
			dataOutputStream.writeInt(doc_vector.size());
			dataOutputStream.writeInt(layerSize);
			float[] syn0 = null;
			for (Entry<Integer, float[]> element : doc_vector.entrySet()) {
				dataOutputStream.writeUTF(element.getKey().toString());
				syn0 = element.getValue();
				for (float d : syn0) {
