...
from caffevis.caffevis_app_state import CaffeVisAppState
from caffevis.caffevis_helper import get_pretty_layer_name, read_label_file, load_sprite_image, load_square_sprite_image, check_force_backward_true
...
class CaffeVisApp(BaseApp):
    ...
    def _draw_status_pane(self, pane):
        ...
        with self.state.lock:
            print('pattern' if self.state.pattern_mode else ('back' if self.state.layers_show_back else 'fwd'),file = status)
            print('%s:%d |' % (self.state.layer, self.state.selected_unit),file = status)
            if not self.state.back_enabled:
                print('Back: off',file = status)
            else:
                print('Back: %s' % ('deconv' if self.state.back_mode == 'deconv' else 'bprop'),file = status)
                print('(from %s_%d, disp %s)' % (self.state.backprop_layer,
                                                           self.state.backprop_unit,
                                                           self.state.back_filt_mode),file = status)
            print('|',file = status)
