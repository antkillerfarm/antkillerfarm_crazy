...
def traceDependentNodes(graph, name, node_input_names, node_map, initializer_map, path_end):
            ...
            if (noutput == name) and (n.name not in node_input_names):
                ...
                if n.name in node_map.keys():
                    for ninput in node_map[n.name].input:
                        if (ninput not in path_end) and (ninput not in initializer_map.keys()):
                            # trace input node's inputs
                            node_input_names = traceDependentNodes(graph, ninput, node_input_names, node_map,
                                                                   initializer_map, path_end)
                        else:
                            print("##" + ninput)
                            node_input_names.append(ninput)
...
def onnx_edit(input_model, output_model, new_input_node_names, input_shape_map, new_output_node_names, output_shape_map,
    ...
    for name in new_names:
        # If a new input name corresponds to an existing node, it implies that original node in the graph needs to be
        # replaced with an input node Exactly here the graph is broken
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inputs",
                        help="comma separated model input names appended with shapes, e.g. --inputs <nodename>[1,2,"
                             "3],<nodename1>[1,2,3] ")
    parser.add_argument("--outputs",
                        help="comma separated model output names appended with shapes, e.g. --outputs <nodename>[1,2,"
                             "3],<nodename1>[1,2,3] ")
